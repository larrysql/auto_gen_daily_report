...
def makeQuaodict(file):
    ...
    f.close()
    return dic
#########################跟据渠道配置文件生成渠道来源字典“bcmm13:手机厂商“
def makeQudaoDistributeDict(file):
    #定义空字典用于存储“bcm13：手机厂商”这样的键值对
    dic = dict()
    f = open(file)
    #逐行读取渠道字典将"bcm*"作为键，手机厂商，web渠道之类的应用渠道作为值写入空字典
    for i in f.readlines():
        n = i.strip('\n').strip('\t').split(':')
        dic[n[-1]]=n[1]
    f.close()
...
def versionAddUserDay(qudao_day_add,day,qdict):   #参数为当天各渠道的新增用户文件，哪一天，渠道字典
    ...
    dictqudao = dict()
    #同一渠道可能有多个渠道代码，qudao_add_user字典就是生成，渠道名和新增用户数直接对应的字典
    qudao_add_user = dict()
    ...
    for key in qdict.keys():
        ...
        if n != y:
            dictqudao[key] = "0"
        y=y+1
    #根据渠道代码和对应的用户增加数字典进行遍历，
    for key in dictqudao.keys():
        #跟据渠道代码在渠道字典查找对应的渠道名，并查看qudao_add_user中有没有，该"渠道名：用户数"
        #键值对，如果没有则将'渠道名：用户数'写入字典，有的话则将键值与本渠道代码对应的新增用户数相加
        #从新存入字典，该过程能将统一渠道名不同渠道代码的新增用户量相加
        if qdict[key] in qudao_add_user.keys():
            qudao_add_user[qdict[key]] = str(int(qudao_add_user[qdict[key]]) + int(dictqudao[key]))
        else:
            qudao_add_user[qdict[key]] = dictqudao[key]
    return  qudao_add_user
...
def makeResultfile(resultfile,dicttoday,dictyesterday):
    ...
    f.close()
#过程同makeResultfile类似
def makeDistributeResultfile(resultfile,dict_distribute):
    fileUtil.cleanFile(resultfile)
    yuanzulist = list()
    for key in dict_distribute.keys():
        yuanzulist.append((key,int(dict_distribute[key])))
    yuanzulist = sorted(yuanzulist, key=lambda num:num[1],reverse = True)
    f = open(resultfile,'a')
    for i in yuanzulist:
        f = open(resultfile,'a')
        a = i[1]
...
#今天渠道新增用户数结果存放的地址
every_qudao_adduser_resultfiletoday = config.get("resultfile","new_user")
#今天渠道来源新增用户数结果存放地址
every_qudao_distribute_adduser_resultfiletoday = config.get("resultfile","new_users_distr") #今天处理结果存放的地址

#根据渠道字典文件生成渠道字典
qudaodict = makeQuaodict(qudaodictfile)
#根据渠道字典文件生成渠道来源字典
qudao_destribute_dict = makeQudaoDistributeDict(qudaodictfile)
...
#生成今天渠道来源的新增用户数字典
qudao_destribute_add_user = versionAddUserDay(qudao_ri_zengliang_datafile_today,today,qudao_destribute_dict)

#将个渠道的今天新增用户数，昨天新增用户数以及增长百分率写入结果文件供主程序调用
makeResultfile(every_qudao_adduser_resultfiletoday,today_qudao_adduser_dict,yesterday_qudao_adduser_dict)
#将渠道来源的新增用户数写入结果文件供主程序调用
makeDistributeResultfile(every_qudao_distribute_adduser_resultfiletoday,qudao_destribute_add_user)
